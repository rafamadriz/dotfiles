...
root_does() {
  ...
  ln -s --force /home/"$user"/.local/share/misc0/root/bashrc /root/.bashrc
  ln -s --force /home/"$user"/.local/share/misc0/root/bash_profile /root/.bash_profile

  # change shell to zsh
  [ "$(echo "$SHELL")" != "/usr/bin/zsh" ] && chsh -s /usr/bin/zsh "$user"

  rm -f packages.yml dotScript.sh
}
root_does
